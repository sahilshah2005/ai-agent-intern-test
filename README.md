...
The policies, products, orders, and 14-document knowledge base are fictional test data.
This is a portfolio prototype, not a deployed customer service system.
...
The agent is designed to avoid unsupported answers. When evidence is insufficient,
it says so and recommends human support; the evaluation suite checks this behavior.
...
python evaluation/run_eval.py --cases all --report-json evaluation/results.json
```
...
The original unit suite contains 320 tests; three additional offline tests check
evaluation reporting. Run the suite above to verify the current result:
...
| `test_eval_reporting.py` | 3 | Multi-turn accounting, failure reporting, incomplete usage |
...
### Reproducible end-to-end report

After setting `OPENAI_API_KEY`, run the full suite with the model specified in
`.env` (`OPENAI_MODEL` defaults to `gpt-4o-mini`):

```bash
python evaluation/run_eval.py --cases all --report-json evaluation/results.json
```

The JSON report records the case pass rate, category totals, failed case IDs and
assertions, p50/p95 wall-clock latency per user turn, API call counts, and input/
output token usage. It excludes full prompts, customer messages, and responses.
For each failed case, the ID and failed assertion show a concrete example to
investigate. Use `--verbose` locally to see response excerpts while debugging.

To estimate **LLM usage cost in USD**, add both prices for the model and billing
terms you used (USD per million tokens):

```bash
python evaluation/run_eval.py --cases all --report-json evaluation/results.json \
  --input-price-per-million 1.0 --output-price-per-million 4.0
```

The numbers in that command are **illustrative inputs**, not current provider
prices. Check your provider's current prices before running it. Cost is an
estimate for reported LLM tokens only: it excludes local embeddings, compute,
failed requests, and any billing adjustments. If a successful API response has
no token usage, the cost field is `null` rather than an incomplete estimate.

**Live results:** Not recorded in this repository. The end-to-end run requires
an API key and makes paid model calls; CI runs offline unit tests only. Do not
quote a pass rate or a cost until the JSON report has been generated by an actual
run. When publishing a result, include the date, model, evaluated commit, suite,
case count, failure IDs, p50/p95 latency, token usage, and the price inputs used.
